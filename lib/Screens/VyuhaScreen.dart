...
// NEW: Imports for web-only features
import 'package:flutter/foundation.dart' show kIsWeb;
// REMOVED: import 'dart:html' as html;

// NEW: Imports for image capture
import 'dart:ui' as ui;
import 'package:flutter/rendering.dart';
import 'dart:typed_data';

// NEW: Import the platform helper
import 'package:vyuha/helpers/platform_helper.dart';

// NEW: Assumed model for collaborators. You must create this file.
...
class _VyuhaScreenState extends State<VyuhaScreen> {
  ...
  final GlobalKey _graphKey = GlobalKey(); // NEW: Key for capturing the graph
  final PlatformHelper _platformHelper = PlatformHelper(); // NEW: Platform helper
  ...
  void initState() {
    ...
    if (widget.roomId.isEmpty) {
      WidgetsBinding.instance.addPostFrameCallback((_) {
        Get.offAllNamed('/home'); // Redirect to home if no roomId
      });
      return;
    }

    // Check if controller already exists to avoid duplicate creation on refresh
    if (!Get.isRegistered<VyuhaController>(tag: widget.roomId)) {
      Get.put(VyuhaController(widget.roomId), tag: widget.roomId);
    }
    _transformationController.addListener(_onTransformationChanged);

    // MODIFIED: Listen for fullscreen changes (like pressing Esc)
    ...
    // MODIFIED: Get initial fullscreen state on web
    if (kIsWeb) {
      setState(() {
  ...
  Widget build(BuildContext context) {
    ...
    // Build the shared actions list once
    final iconColor = _isDarkMode ? Colors.white70 : Colors.black54;
    final List<Widget> actions = _buildActions(ctrl, iconColor);

    return SafeArea(
      // Added SafeArea for mobile
      // UPDATED: Wrap with WillPopScope to handle system back button
      child: WillPopScope(
        onWillPop: () async {
        ...
        child: Scaffold(
          ...
          // MODIFIED: Add a conditional AppBar for mobile
          ...
          body: Obx(() {
            ...
            return Stack(
              children: [
                ...
                // MODIFIED: Only show Positioned controls on large screens
                ...
                if (!_isSearching) // Zoom controls are fine on all screens
                  _ZoomControls(
                    isDarkMode: _isDarkMode,
  ...
  // MODIFIED: Method to toggle fullscreen
  void _toggleFullScreen() {
    if (!kIsWeb) return;

    _platformHelper.toggleFullScreen();
    // The listener in initState will automatically update the state
  }
  ...
  // NEW: Extracted list of action widgets to avoid duplication
  List<Widget> _buildActions(VyuhaController ctrl, Color iconColor) {
    return [
      ...
      // MODIFIED: Add fullscreen button only for web
      if (kIsWeb)
        _buildToolbarButton(
      ...
      // NEW: Add Collaborator Management Button
      Obx(() {
        if (ctrl.isOwner.value) {
          return _buildToolbarButton(
            Icons.group_outlined, // New icon
            'Manage Collaborators', // New tooltip
            iconColor,
            () => _showCollaboratorDialog(context, ctrl), // New dialog
          );
        }
        return SizedBox.shrink();
      }),
  ...
  // --- (COMPLETELY REBUILT) Method to show and manage collaborators and banned users ---
  ...
  Future<void> _showAddDialog(
    ...
    await showDialog(
      ...
      builder: (c) => AlertDialog(
        ...
        // --- FIX START ---
        // Wrap content in a Container to constrain width on web
        content: Container(
  ...
  // MODIFIED: This dialog now shows AI limits and disables the button
      ...
      builder: (c) => AlertDialog(
        ...
        content: Container(
          ...
          child: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
              // NEW: Add Obx to show remaining uses
              Obx(() {
              ...
              TextField(
                ...
                style: TextStyle(color: mainText),
                decoration: InputDecoration(
                  labelText: 'Number of ideas (1-9)', // MODIFIED label
                  labelStyle: TextStyle(color: subText),
                  enabledBorder: OutlineInputBorder(
                    borderSide: BorderSide(color: borderColor),
                    borderRadius: BorderRadius.circular(8),
                ...
                // NEW: Add input formatters to limit to 1 digit (1-9)
                inputFormatters: [
                  FilteringTextInputFormatter.digitsOnly,
                  LengthLimitingTextInputFormatter(1),
                  TextInputFormatter.withFunction((oldValue, newValue) {
                    if (newValue.text == '0') return oldValue; // Disallow '0'
                    return newValue;
                  }),
                ],
              ),
        ...
        actions: [
          ...
          // MODIFIED: Wrap button in Obx and add conditional onPressed
          Obx(() => ElevatedButton.icon(
                ...
                onPressed: ctrl.aiUsesRemaining.value > 0
                    ? () async {
                        ...
                        // --- CHANGE START ---
                        // Modified logic to check for valid topic and count
                        if (t.isNotEmpty && count > 0) {
                          ...
                          try {
                            ...
                            // NEW: Show error from controller
                            _showCustomNotification(
  ...
  // MODIFIED: Method to capture and save/share the graph as an image
  Future<void> _exportVyuhaAsImage(BuildContext context) async {
    ...
    try {
      ...
      if (kIsWeb) {
        // On web, use the helper to trigger a download
        await _platformHelper.saveImage(pngBytes, 'Vyuha-Export.png');
        _showCustomNotification('Image export started');
      } else {
        // --- CHANGE START: Mobile Sharing ---
        // On mobile, save temporarily and then share
...
// -------------------------------------------------------------------
// --- UI Components (Refactored from _VyuhaScreenState) ---
// -------------------------------------------------------------------
  ...
  Widget build(BuildContext context) {
    ...
    return AppBar(
      ...
      title: Obx(() {
        ...
        // Truncate title differently for app bar
...
/// **Top-left controls for Web** (Title, Stats)
      ...
      child: Container(
        ...
        child: Row(
          children: [
            ...
            if (isLargeScreen) ...[
              InkWell(
                // UPDATED: Use offAllNamed to go home
                onTap: () {
                  Get.offAllNamed('/home'); // Navigate back to HomeScreen
                },
                borderRadius: BorderRadius.circular(8),
                child: Padding(
            ...
            if (isLargeScreen) ...[
              ...
              Obx(() => Row(
                    ...
                    children: [
                      ...
                      SizedBox(width: 16),
                      // UPDATED: Use collaborators list for unique authors
                      _statChip(
                      ...
                      // NEW: AI Usage Stat Chip for web
                      SizedBox(width: 16),
...
/// **Top-right controls for Web** (Actions)
  ...
  Widget build(BuildContext context) {
    ...
    return Positioned(
      ...
      child: Container(
        ...
        child: Row(
          // MODIFIED: Use the new _buildActions method
          children: actions,
        ),
      ),
    );
...
/// **Empty state widget** (shown when no nodes exist)
...
class _GraphWidget extends StatelessWidget {
  ...
  constrained: false, // This tells the InteractiveViewer to provide infinite space
  ...
  child: RepaintBoundary(
    ...
    // REMOVE the Container and Center.
    // Let the GraphView be the direct child so it can
    // be as large as it needs to be.
    child: GraphView(
      graph: graph,
